...
export function ColaboradorModal({
  ...
  const handleGenerateTermo = async () => {
    ...
    const safePlaca = sanitizeFileName(payload.plc)
    const fileName = `Termo_${safeName || "colaborador"}_${safePlaca || "veiculo"}.docx`

    try {
      const res = await fetch("/api/termo", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        toast({
          title: "Erro",
          description: data?.error || "Nao foi possivel gerar o termo.",
          variant: "destructive",
        })
        return
      }
      ...
      toast({ title: "Sucesso", description: "Termo gerado com sucesso." })
    } catch {
      toast({
        title: "Erro",
        description: "Nao foi possivel gerar o termo.",
        variant: "destructive",
      })
    }
  }
